  ...
  return (
    <div className="overflow-hidden">
      ...
      {/* ══════════════════════════════════════
          HERO SECTION
      ══════════════════════════════════════ */}
      <section className="relative bg-dark min-h-[92vh] flex items-center overflow-hidden">
        {/* Background image */}
        <div
        ...
        />
        {/* Layered gradients */}
        <div className="absolute inset-0 bg-gradient-to-br from-dark via-dark/85 to-brand-900/70" />
        <div className="absolute inset-0 bg-gradient-to-t from-dark/60 via-transparent to-transparent" />
        {/* Animated glow orbs */}
        <div className="absolute top-1/4 right-[15%] w-72 h-72 rounded-full bg-brand-500/10 blur-3xl animate-float" />
        <div className="absolute bottom-1/4 left-[10%] w-96 h-96 rounded-full bg-brand-400/8 blur-3xl animate-float-delay" />
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-20">
        <div className="flex items-end justify-between mb-10 reveal">
          <div>
            <p className="text-sm font-bold text-brand-500 mb-2 uppercase tracking-widest">Handpicked for you</p>
            <h2 className="font-display text-4xl font-bold text-dark">Featured Dishes</h2>
          </div>
          <Link
            to="/menu"
            className="hidden sm:inline-flex items-center gap-2 text-sm font-bold text-brand-500 hover:text-brand-600 transition-colors group"
          >
            View full menu
        ...
        {loading ? (
          ...
          <div className="text-center py-20 rounded-3xl bg-gray-50/80 border border-gray-100 reveal">
            <div className="text-5xl mb-4">🍽️</div>
            <p className="font-display text-xl font-semibold text-dark/40 mb-2">Menu coming soon</p>
            <p className="text-sm text-dark/30">Check back shortly — we're preparing something special.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {items.map((item, i) => (
              <div key={item.id} className={`reveal-scale delay-${Math.min(i * 100, 600)}`}>
        ...
        <div className="mt-8 text-center sm:hidden reveal">
          <Link to="/menu" className="inline-flex items-center gap-2 text-sm font-bold text-brand-500 hover:text-brand-600">
            View full menu →
          </Link>
        </div>
      </section>
      ...
      <section className="bg-dark py-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16 reveal">
            <p className="text-sm font-bold text-brand-400 mb-3 uppercase tracking-widest">Why FoodApp?</p>
            <h2 className="font-display text-4xl sm:text-5xl font-bold text-white mb-4">Built for food lovers</h2>
            <p className="text-white/40 max-w-md mx-auto text-base">We obsess over every detail so you can just enjoy your meal.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {WHY_US.map((f, i) => (
              <div
                key={f.title}
                className={`group rounded-3xl bg-white/5 border border-white/8 p-8 hover:bg-white/10 transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_12px_40px_rgba(0,0,0,0.3)] reveal`}
                style={{ transitionDelay: `${i * 150}ms` }}
              >
                <div className={`inline-flex h-14 w-14 items-center justify-center rounded-2xl mb-6 transition-colors duration-300 ${f.color} ${f.colorHover}`}>
                  {f.icon}
                </div>
